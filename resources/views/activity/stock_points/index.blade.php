...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h5 class="py-3 mb-0"><span class="text-muted fw-light">Masters /</span> Stocks Points List</h5>
    @if(hasPermission(['add_stock_points']))
    <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addDealerModal">
        <i class="mdi mdi-plus me-1"></i> Stocks Points
    </button>
    @endif
</div>
            ...
            <table class="table table-bordered table-hover" id="dealersTable">
                <thead class="table-light">
                    <tr>
                        ...
                        <th>Points</th>
                        <th>Created At</th>
                        @if(hasPermission(['edit_stock_points','delete_stock_points']))
                        <th>Actions</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                    ...
                    @foreach($stock_details as $key =>$point)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $point->kg ?? 'N/A'}}</td>
                        <td>{{ $point->points ?? 'N/A'}}</td>
                        <td>{{ \Carbon\Carbon::parse($point->created_at)->format('d-m-Y') ?? 'N/A'}}</td>

                        @if(hasPermission(['edit_stock_points','delete_stock_points']))
                        <td>
                            <div class="d-flex gap-2">
                                @if(hasPermission(['edit_stock_points']))
                                ...
                                @if(hasPermission(['delete_stock_points']))
                                <form id="delete-form-{{ $point->id }}" action="{{ route('activity.points.destroy', $point->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete({{ $point->id }})" class="btn btn-sm btn-icon btn-outline-danger">
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="addDealerModalLabel">Stocks Points</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <form action="{{ route('activity.points.store') }}" method="POST">
                @csrf
                <div class="modal-body">
                    <!-- Example fields -->
                    <div class="mb-3">
                        <label for="dealer_name" class="form-label">Kg</label>
                        <input type="text" class="form-control" id="kg" name="kg" required>
                        @error('kg') <div class="text-danger mt-1 small">{{ $message }}</div> @enderror
                    </div>
                    <div class="mb-3">
                        <label for="dealer_name" class="form-label">Points</label>
...
<script>
    ...
    (function($) {
        $(document).ready(function() {
            ...
            $('#dealersTable').DataTable({
                ...
                // columnDefs: [{
                //         targets: 3, // Created At column (index 9)
                //         type: 'date-dd-mm-yyyy'
                //     },
                //     {
                //         targets: 4, // Actions column (index 10)
                //         orderable: false,
                //         searchable: false
                //     }
                // ],
    ...
    function confirmDelete(id) {
        Swal.fire({
            title: 'Delete this Stock Points?',
            icon: 'warning',
