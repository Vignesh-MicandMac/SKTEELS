...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h5 class="py-3 mb-0"><span class="text-muted fw-light">Masters /</span> Promotor Type List</h5>
    @if(hasPermission(['add_promotors_type']))
    <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addDealerModal">
        <i class="mdi mdi-plus me-1"></i> Add Promotor Type
    </button>
    @endif
</div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover" id="dealersTable">
                <thead class="table-light">
                    <tr>
                        <th>ID</th>
                        <th>Promotor Type Name</th>
                        <th>Is Active</th>
                        <th>Created At</th>
                        @if(hasPermission(['edit_promotors_type','delete_promotors_type']))
                        <th>Actions</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        <td>{{ $promotor_type->id }}</td>
                        <td>{{ $promotor_type->promotor_type ?? 'N/A'}}</td>
                        <td>
                            @if($promotor_type->is_active == 1)
                            <span class="badge bg-success">Active</span>
                            @elseif($promotor_type->is_active == 0)
                            <span class="badge bg-danger">Inactive</span>
                            @else
                            <span class="badge bg-secondary">N/A</span>
                            @endif
                        </td>
                        <td>{{ \Carbon\Carbon::parse($promotor_type->created_at)->format('d-m-Y') ?? 'N/A'}}</td>

                        @if(hasPermission(['edit_promotors_type','delete_promotors_type']))
                        <td>
                            <div class="d-flex gap-2">
                                @if(hasPermission(['edit_promotors_type']))
                                <a href="javascript:void(0);"
                                    class="btn btn-sm btn-icon btn-outline-primary editPromotorTypeBtn"
                                    data-id="{{ $promotor_type->id }}">
                                    <i class="mdi mdi-pencil"></i>
                                </a>
                                @endif


                                @if(hasPermission(['delete_promotors_type']))
                                <form id="delete-form-{{ $promotor_type->id }}" action="{{ route('masters.pro_type.destroy', $promotor_type->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete({{ $promotor_type->id }})" class="btn btn-sm btn-icon btn-outline-danger">
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="addDealerModalLabel">Add Promotor Type</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <form action="{{ route('masters.pro_type.store') }}" method="POST">
                @csrf
                <div class="modal-body">
                    <!-- Example fields -->
                    <div class="mb-3">
                        <label for="dealer_name" class="form-label">Promotor Type Name</label>
                        <input type="text" class="form-control" id="role_name" name="promotor_type" required>
...
<script>
    ...
    function confirmDelete(id) {
        Swal.fire({
            title: 'Delete this Promotor Type?',
            icon: 'warning',
    ...
    $(document).on('click', '.editPromotorTypeBtn', function() {
        ...
        $.ajax({
            url: `/masters/promotors_type/edit/${id}`,
            type: 'GET',
