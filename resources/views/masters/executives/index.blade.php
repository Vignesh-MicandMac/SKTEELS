...
@section('title', 'Executives List')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h5 class="py-3 mb-0"><span class="text-muted fw-light">Masters /</span> Executives List</h5>
    @if(hasPermission(['add_executives']))
    <a href="{{ route('masters.executives.create') }}" class="btn btn-primary btn-sm">
        <i class="mdi mdi-plus me-1"></i> Add Executive
    </a>
    @endif
</div>
            ...
            <table class="table table-bordered table-hover" id="dealersTable">
                <thead class="table-light">
                    <tr>
                        ...
                        <th>District</th>
                        <th>Created At</th>
                        @if(hasPermission(['edit_executives','delete_executives']))
                        <th>Actions</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $executive->states->state_name ?? 'N/A' }}</td>
                        <td>{{ $executive->districts->district_name ?? 'N/A'}}</td>
                        <td>{{ \Carbon\Carbon::parse($executive->created_at)->format('d-m-Y') ?? 'N/A'}}</td>

                        @if(hasPermission(['edit_executives','delete_executives']))
                        <td>
                            <div class="d-flex gap-2">
                                @if(hasPermission(['edit_executives']))
                                <a href="{{ route('masters.executives.edit', $executive->id) }}" class="btn btn-sm btn-icon btn-outline-primary">
                                    <i class="mdi mdi-pencil"></i>
                                </a>
                                @endif

                                @if(hasPermission(['delete_executives']))
                                <form id="delete-form-{{ $executive->id }}" action="{{ route('masters.executives.destroy', $executive->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
    ...
    (function($) {
        $(document).ready(function() {
            ...
            $('#dealersTable').DataTable({
                ...
                // columnDefs: [{
                //         targets: 5, // Created At column (index 9)
                //         type: 'date-dd-mm-yyyy'
                //     },
                //     {
                //         targets: 6, // Actions column (index 10)
                //         orderable: false,
                //         searchable: false
                //     }
                // ],
