...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h5 class="py-3 mb-0"><span class="text-muted fw-light">Masters /</span> Roles List</h5>
            ...
            <table class="table table-bordered table-hover" id="dealersTable">
                <thead class="table-light">
                    <tr>
                        ...
                        <th>Name</th>
                        <th>Created At</th>
                        @if(hasPermission(['edit_roles','delete_roles']))
                        <th>Actions</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        <td>{{ $role->id }}</td>
                        <td>{{ $role->name ?? 'N/A'}}</td>
                        <td>{{ \Carbon\Carbon::parse($role->created_at)->format('d-m-Y') ?? 'N/A'}}</td>
                        ...
                        <td>
                            <div class="d-flex gap-2">
                                ...
                                <a href="javascript:void(0);"
                                    class="btn btn-sm btn-icon btn-outline-primary editRoleBtn"
                                    data-id="{{ $role->id }}">
                                ...
                                <form id="delete-form-{{ $role->id }}" action="{{ route('users.role.destroy', $role->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete({{ $role->id }})" class="btn btn-sm btn-icon btn-outline-danger">
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="addDealerModalLabel">Add Role</h5>
            ...
            <form action="{{ route('users.role.store') }}" method="POST">
                ...
                <div class="modal-body">
                    <!-- Example fields -->
                    <div class="mb-3">
                        <label for="dealer_name" class="form-label">Role Name</label>
...
<div class="modal fade" id="editDealerModal" tabindex="-1" aria-labelledby="editDealerModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content" id="editRoleModalContent">
    ...
    $(document).on('click', '.editRoleBtn', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#editRoleModalContent').html(response);
